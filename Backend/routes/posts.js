...
function toClientPost(post) {
  ...
  if (post.result) {
    results = post.targets.map((platform) => {
      const r = post.result[platform];
      if (!r) {
        return { platform, success: false, error: 'Not attempted.' };
      }
      return r.success
        ? { platform, success: true, postId: r.data?.id || r.data?.post_id || null }
        : { platform, success: false, error: r.error };
    });
