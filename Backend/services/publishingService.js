...
async function publishPost(userId, pageId, post) {
  ...
  function describeGraphError(err) {
    ...
    console.error('[publishPost] Graph API call failed:', {
    ...
    if (graphError?.code === 190) {
      reauthNeeded = true;
      reauthMessage = graphError.message;
    }
    return graphError?.message
      ? `${graphError.message}${graphError.error_subcode ? ` (subcode ${graphError.error_subcode})` : ''}`
      : `${err.message} (no structured error body from Graph API — check Render logs)`;
  }

  if (post.targets.includes('facebook')) {
    try {
      const data = await meta.publishFacebookPost(account.pageId, pageAccessToken, {
        message: post.message,
        link: post.link,
        imageUrl: post.imageUrl
      });
      results.facebook = { success: true, data };
    } catch (err) {
      results.facebook = { success: false, error: describeGraphError(err) };
    }
  }
  ...
  if (post.targets.includes('instagram')) {
    if (!account.instagramBusinessAccountId) {
      ...
      try {
        const data = await meta.publishInstagramPost(
        ...
        results.instagram = { success: true, data };
      } catch (err) {
        results.instagram = { success: false, error: describeGraphError(err) };
      }
    }
  }

  if (reauthNeeded) {
    await markNeedsReauth(account, reauthMessage);
    const reauthErr = new Error('Token invalid or revoked. User must reconnect via OAuth.');
    reauthErr.code = REAUTH_REQUIRED;
    reauthErr.results = results;
    throw reauthErr;
  }
  ...
  if (anyFailed) {
    ...
    account.lastError = Object.values(results).find((r) => !r.success)?.error || 'One or more targets failed.';
