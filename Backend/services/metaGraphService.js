 ...
 * FIX: sends data in the request body (not URL query params) to avoid
 * "Please reduce the amount of data you're asking for" errors caused by
 * long image URLs pushing the request over Facebook's URL length limits.
 */
async function publishFacebookPost(pageId, pageAccessToken, payload) {
  ...
  if (imageUrl) {
    ...
    const { data } = await axios.post(`${GRAPH_BASE}/${pageId}/photos`, body);
  ...
  const { data } = await axios.post(`${GRAPH_BASE}/${pageId}/feed`, body);
  return data; // { id }
}

/**
 * FIX: Instagram needs a moment to download and process a media container
 * (especially video/reels, but sometimes images too) before it can be
 * published. Publishing too early causes "Media ID is not available"
 * (subcode 2207027). This polls status_code until it's FINISHED (or bails
 * out on ERROR / timeout).
 */
async function waitForContainerReady(containerId, pageAccessToken, { maxAttempts = 15, delayMs = 2000 } = {}) {
  ...
  throw new Error('Instagram media container took too long to process (timed out waiting for FINISHED status).');
}
